...
__global__ void computeAccelerationMatrix(vector3 *accels, vector3 *hPos,
                                          double *mass) {
...
__global__ void updateVelocityPosition(vector3 *accels, vector3 *hPos,
                                       vector3 *hVel) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
...
void compute(vector3 *hPos, vector3 *hVel, double *mass) {
  ...
  cudaMemcpy(d_hPos, hPos, sizeof(vector3) * NUMELEMENTS,
             cudaMemcpyHostToDevice);
  cudaMemcpy(d_hVel, hVel, sizeof(vector3) * NUMELEMENTS,
             cudaMemcpyHostToDevice);
  cudaMemcpy(d_mass, mass, sizeof(double) * NUMELEMENTS,
             cudaMemcpyHostToDevice);
  ...
  cudaMemcpy(hPos, d_hPos, sizeof(vector3) * NUMELEMENTS,
             cudaMemcpyDeviceToHost);
  cudaMemcpy(hVel, d_hVel, sizeof(vector3) * NUMELEMENTS,
             cudaMemcpyDeviceToHost);
