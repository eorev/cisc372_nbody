...
void compute() {
  ...
  // Launch calcAccels kernel to compute accelerations based on
  // gravitational forces
  calcAccels<<<gridBlocks, gridThreads>>>(d_accels, d_hPos, d_mass);

  // Set up dimensions for sumCols kernel
  int threadCount = 64;
  ...
  sumCols<<<blockDimension, threadCount, sharedMemSize>>>(d_accels,
                                                          d_accel_sum);

  // Set up dimensions for updatePosAndVel kernel
...
/**
 * CUDA Kernel Function - updatePosandVel
 ...
 */
__global__ void sumCols(vector3 **accels, vector3 *accelTotals) {
...
/**
 * CUDA Kernel Function - calcAccels
 ...
 */
__global__ void calcAccels(vector3 **accels, vector3 *positions,
                           double *masses) {
