...
__global__ void computeAccelerationMatrix(vector3 *accels, vector3 *d_hPos,
  ...
  if (threadIdx.y == 0 && j < NUMELEMENTS) {
    sharedMass[threadIdx.x] = (threadIdx.x < BLOCK_SIZE) ? d_mass[j] : 0.0;
  }

  if (i < NUMELEMENTS && j < NUMELEMENTS) {
    vector3 pos = {d_hPos[j][0], d_hPos[j][1], d_hPos[j][2]};
    sharedPos[threadIdx.y][threadIdx.x][0] = pos[0];
    sharedPos[threadIdx.y][threadIdx.x][1] = pos[1];
    sharedPos[threadIdx.y][threadIdx.x][2] = pos[2];
  }
  __syncthreads();
